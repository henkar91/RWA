...
pdata <- df %>% 
    mutate(xmean = mean(importance, na.rm = TRUE),
...
ggplot(data = pdata) +
    ...
    geom_vline(aes(xintercept = xmean), color = "grey", linetype = 2, size = 0.3) +
    geom_hline(aes(yintercept = ymean * 100), color = "grey", linetype = 2, size = 0.3)
